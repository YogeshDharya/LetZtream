import {Button,TextField,Box,Stack,MenuItem,FormControl, FormHelperText} from '@mui/material';
//InputLabel and Select ?? 
...
const UploadVideo =(props)=> {
  ...
  const [formFields, setFormFields] = useState({title:"",videoLink:"",thumbnailImageLink: "",age: "",genre: "", date:null});
  ...
  const handleChange = (e)=>{
    ...
    if(name==="videoLink"){
      ...
      setFormFields({...formFields,videoLink:value,thumbnailImageLink:`https://i.ytimg.com/vi/${id}/maxresdefault.jpg`})
    }
  ...
  const validateInputs = (formFields,date,embedURL)=>{
    ...
    if(embedURL==="INVALID_URL"){
      enqueueSnackbar("The Video Link provided is Invalid!", {variant:"error"});
      return false;
    }
    else if(!formFields.thumbnailImageLink.includes("https://")){
      enqueueSnackbar("Copy paste the video link here as well", {variant:"error"});
      return false;
    }
    else if(formFields.genre===""){
      enqueueSnackbar("You need to choose a valid genre from the dropdown", {variant:"error"});
      return false;
    }
    else if(formFields.age===""){
      enqueueSnackbar("You need to select a valid Content Rating from the dropdown ", {variant:"error"});
      return false;
    }
  ...
  const convertURLToEmbeddedURL=(url)=>{
    ...
    if(url.includes("https://www.youtube.com/watch?v=")){
      embedURL = url.replace("https://www.youtube.com/watch?v=","youtube.com/embed/");
    }
    ...
    <div>
      ...
      >
        <DialogTitle className="dialog-title">
            ...
            <Box><Button startIcon={<CloseIcon fontSize="large"/>} onClick={handleClose}></Button></Box>
        </DialogTitle>
        <DialogContent className="dialog-content">
        <Stack spacing={2} sx={{p:0.5}}>
        <TextField name="videoLink" value={formFields.videoLink} className="form-element" label="Video Link" variant="outlined" helperText="This link will be used to derive the video" sx={{borderColor:"#ffff"}} onChange={(e)=>{handleChange(e)}} required/>
        <TextField name="thumbnailImageLink" value={formFields.thumbnailImageLink} label="Thumbnail Image Link" variant="outlined" helperText="This link will be used to preview the thumbnail image" onChange={(e)=>{handleChange(e)}} required/>
        <TextField name="title" value={formFields.title} label="Title" variant="outlined" helperText="This title will be representative text for the video" onChange={(e)=>{handleChange(e)}} required/>
        <TextField
        ...
        <FormControl>
          <DatePicker 
              ...
              onChange={(newValue)=>{setFormFields({...formFields,date:newValue})}}
        ...
        <div className="dialog-actions">
          <Button sx={{
            ...
            onClick={(e)=>{handleUploadClick(formFields)}} 
            variant="contained"
            >
              Upload
            </Button>
